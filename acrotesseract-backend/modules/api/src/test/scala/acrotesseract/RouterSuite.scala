...
class RouterSuite extends munit.FunSuite:
  ...
  private val ground = Pose(id(1), "Ground", None, None)
  private val bird = Pose(id(2), "Front Bird", None, Some("Horizontal on the base's feet."))
  private val throne = Pose(id(3), "Throne", None, None)
  ...
  private val router = Router(
    ...
    InMemoryRepository(
      ...
      List(
        Transition(id(11), "Ground to Front Bird", None, ground.id, bird.id, Some("https://youtu.be/g8OhDBRwhSw?t=403")),
        Transition(id(12), "Front Bird to Throne", None, bird.id, throne.id, None),
        Transition(id(13), "Throne to Front Bird", None, throne.id, bird.id, None)
      )
    )
  )
  ...
  test("GET /api/poses/{id} includes transitions from and to the pose, with UUID ids"):
    ...
    assertEquals(
      res.body,
      s"""{"pose":{"id":"${id(2)}","name":"Front Bird","descriptionMd":"Horizontal on the base's feet."},""" +
        s""""transitionsFrom":[{"id":"${id(12)}","name":"Front Bird to Throne","poseFrom":"${id(2)}","poseTo":"${id(3)}"}],""" +
        s""""transitionsTo":[{"id":"${id(11)}","name":"Ground to Front Bird","poseFrom":"${id(1)}","poseTo":"${id(2)}","youtubeUrl":"https://youtu.be/g8OhDBRwhSw?t=403"},""" +
        s"""{"id":"${id(13)}","name":"Throne to Front Bird","poseFrom":"${id(3)}","poseTo":"${id(2)}"}]}"""
    )
  ...
  test("GET /api/transitions/{id} includes both poses"):
    ...
    assert(res.body.contains(s""""poseTo":{"id":"${id(3)}","name":"Throne"}"""), res.body)
